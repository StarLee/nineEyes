...
<script type="text/javascript">
	...
	function insertLinkNews() {
		...
		$(inputPicURL).attr("value",
				$("input[name='dialog_linkNewsName']").attr("value"));
		var inputPicType = document.createElement("input");
		$(inputPicType).attr("type", "hidden");
		$(inputPicType).attr("name", "linkNewsURL");
		$(inputPicType).attr("value",
				$("input[name='dialog_linkNewsURL']").attr("value"));
			...
			<div class="content">
				<div class="rightcontent">
					<div class="ApplyForm leftContentMiddle ">
						<form action="${root}/UpdateEvent.do" method="post"
							id="contentForm">
							...
							<table>
								<tr>
									...
									<td align="left" valign="middle"><input type="text"
										name="title" value="${event.title }"><select
										name="category">
								...
								<tr>
									<td>概要</td>
									<td><textarea rows="2" cols="50" name="outline">${event.outline }</textarea>
									</td>
								</tr>
							</table>
								...
								<div>
									<eyes:myCK>
										<ckeditor:editor basePath="${root}/ckeditor/"
											...
											value="${event.intro }" events="${defaultEvent}" />
									</eyes:myCK>
								</div>
